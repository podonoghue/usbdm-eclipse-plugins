...
////            if (monitoredVariable.getKey().contains("enableGettersAndSetters")) {
...
//    * @param reference        Controlling expression
//    *
//    * @return Variable update information from evaluating expression etc.
//    *
//    * @throws Exception
//    */
//   private VariableUpdateInfo determineVariableUpdate(Variable controlVar, Variable targetVariable, Expression reference) throws Exception {
//
//      VariableUpdateInfo info = new VariableUpdateInfo();
//
//      info.value = reference.getValue();
//
//      if (reference.isConstant()) {
//         // Reference is unchanging
//         if ((targetVariable != null) && !(boolean)reference.getValue()) {
...
//         return info;
//      }
//      Variable primaryVariableInExpression = reference.getPrimaryVar();
//
...
//         }
//         else {
//            info.origin = info.origin + "\n[modified by " + "*** To be done  #1***" +"]";
////            info.origin = info.origin + "\n[modified by " + reference.getCollectedIdentifiers() +"]";
//         }
//      }
//      else {
...
//    * @throws Exception
//    */
//   private VariableUpdateInfo updateTarget(Variable controlVar, Variable targetVariable, Expression expression) throws Exception {
//
...
////            String exprStr       = fChoiceVariable.getSelectedItemData().getReference();
////            String expressions[] = exprStr.split(";");
////
...
////                  continue;
////               }
////               Expression exp = new Expression(expressions[index], fChoiceVariable.getProvider());
////               updateTarget(fChoiceVariable, safeGetVariable(targets[index].trim()), exp);
////            }
////         } catch (Exception e) {
...
//            Expression reference = fControlledVar.getReference();
//            if (reference != null) {
...
//            }
//            // Reference of form "[varName];expression"
//            Expression enabledByExpression = fControlledVar.getEnabledBy();
//            if (enabledByExpression != null) {
//               enabledByExpression.evaluateAsEnabledBy(info);
//            }
//
//            Expression errorIfExpression = fControlledVar.getErrorIf();
//            if (errorIfExpression != null) {
//               if ((boolean) errorIfExpression.getValue()) {
//                  info.status = new Status(fControlledVar.getErrorIfMessage());
//               }
//            }
//            // Reference of form "[varName],expression"
//            Expression unlockedByExpression = fControlledVar.getUnlockedBy();
//            if (unlockedByExpression != null) {
//               fControlledVar.setLocked(!(boolean)unlockedByExpression.getValue());
//            }
...
//    *
//    * @param enabledByExpression    String of form "expression" or "primaryVar#expression"
//    * @param listener               Listener to add to expression
...
//    * @throws Exception
//    */
//   private boolean addExpressionListeners(Expression expression, IModelChangeListener listener, Variable owningVar) throws Exception {
//      /*
...
//      return true;
////      ArrayList<String> identifiers = expression.getVariables();
...
//    * @throws Exception
//    */
//   private boolean addExpressionListeners(String expression, IModelChangeListener listener, Variable owningVar) throws Exception {
//      /*
//       * Need to make sensitive to:
//       *  - Each referenced variable in expression
//       *  - Clock selection if any referenced variable is dependent on active clock selection
//       */
//      if ("disabled".equalsIgnoreCase(expression)) {
//         return false;
//      }
//      if (expression.contains("#")) {
//         throw new Exception("Illegal expression "+expression);
//      }
...
//            // Allowed to have no target for self updating clock selection
//         }
//         else {
//            // Sanity check target
//            Variable target  = safeGetVariable(targetName);
...
//
//      Expression controlledByExpression = controlledVariable.getReference();
//
//      if (controlledByExpression == null) {
//         return false;
//      }
//
//      // Get/create listener to link references to target
...
//      return addExpressionListeners(controlledByExpression, listener, controlledVariable);
//   }
//
//   /**
...
//
//      Expression enabledByExpression = enabledVariable.getEnabledBy();
//      if (enabledByExpression == null) {
//         return false;
//      }
//      // Get/create listener to link references to target
...
//
//      if (enabledByExpression.isConstant()) {
//         // Don't watch constant expressions
//         return false;
//      }
//      return addExpressionListeners(enabledByExpression, listener, enabledVariable);
//   }
//
//   /**
...
//
//      Expression errorIfExpression = erroredVariable.getErrorIf();
//      if (errorIfExpression == null) {
//         return false;
//      }
//      // Get/create listener to link references to target
...
//      return addExpressionListeners(errorIfExpression, listener, erroredVariable);
//   }
//
//   /**
//    * This handles a variable that is enabled by other variables
...
//
//      Expression unlockedByExpression = unlockedVariable.getUnlockedBy();
//      if (unlockedByExpression == null) {
//         return false;
//      }
//      // Get/create listener to link references to target
...
//      return addExpressionListeners(unlockedByExpression, listener, unlockedVariable);
//   }
//
//   /**
...
////      }
//      Expression dynamicExpression = dynamicMinMaxVariable.getMaxExpression();
...
//      // No need to check for duplicates as this would require multiple of the same dependent variable
//      return addExpressionListeners(dynamicExpression, getOrCreateControlledByListener(dynamicMinMaxVariable), dynamicMinMaxVariable);
//   }
//
//   /**
...
//         Expression dynamicExpression = choice.getEnabledBy();
//         if (dynamicExpression == null) {
//            continue;
//         }
//         // Create listener to changes in choices of target
//         dynamicValueFound = addExpressionListeners(dynamicExpression, listener, dynamicChoiceVariable);
//      }
