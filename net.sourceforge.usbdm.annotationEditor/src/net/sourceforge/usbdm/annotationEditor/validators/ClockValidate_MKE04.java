...
public class ClockValidate_MKE04 extends MyValidator {
   ...
   public void validate(TreeViewer viewer) throws Exception {
      ...
      Message system_timer_clockMessage = 
            new Message(String.format("Must be <= %2.1f MHz.", MAX_TIMER_CLOCK_FREQ/1000000.0), Severity.INFORMATION);
      if (system_timer_clock > MAX_TIMER_CLOCK_FREQ) {
         system_timer_clockMessage = 
               new Message(String.format("Clock frequency is too high. (Req. clock <= %2.2f MHz)", MAX_TIMER_CLOCK_FREQ/1000000.0));
      }
