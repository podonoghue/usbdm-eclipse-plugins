...
//                System.err.println("Include path getLocation            = "+projectHandle.getFolder(targetPath).getLocation());
//                System.err.println("Include path getFullPath            = "+projectHandle.getFolder(targetPath).getFullPath());
//                System.err.println("Include path getProjectRelativePath = "+projectHandle.getFolder(targetPath).getProjectRelativePath());

//                String path = "\"${workspace_loc:" + projectHandle.getFolder(targetPath).getFullPath().toOSString() + "}\""
                
			    // Creates include folder path that is portable (e.g. rename project doesn't break paths)
			    String path = "\"${ProjDirPath}/" + projectHandle.getFolder(targetPath).getProjectRelativePath().toOSString() + "\"";
			...
			throw new ProcessFailureException("Create Include Folder failure: while setting include path: " + e.getMessage(), e); //$NON-NLS-1$
		}
