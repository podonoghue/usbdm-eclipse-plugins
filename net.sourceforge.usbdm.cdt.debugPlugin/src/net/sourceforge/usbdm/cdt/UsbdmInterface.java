...
//	   System.err.println("UsbdmInterface.doRemote(\'"+connection+"\')");
      if (connection != null) {
         ...
         String targetDevice;
         if (separatorIndex < 0) {
            spritePath   = trimmedConnection;
            targetDevice = "";
         }
         else {
            spritePath   = escapeScpaces(trimmedConnection.substring(0, separatorIndex).trim());
            targetDevice = trimmedConnection.substring(separatorIndex+1);
         }
         try {
//          System.err.println("UsbdmInterface.doRemote() Before substitution spritePath = \'" + spritePath + "\"");
            IStringVariableManager manager = VariablesPlugin.getDefault().getStringVariableManager();
            spritePath = manager.performStringSubstitution(spritePath);
//          System.err.println("UsbdmInterface.doRemote() After substitution spritePath  = \'" + spritePath + "\"");
         } catch (CoreException e1) {
            e1.printStackTrace();
         }
         spritePath = spritePath.replace("\\", "/");
//	      System.err.println("UsbdmInterface.doRemote(\'"+spritePath+"\' + \'"+targetDevice+"\')");
         addCmd(commands, "set remotetimeout 3000");                                      //$NON-NLS-1$
         addCmd(commands, "-target-select remote | \"" + spritePath  + "\" " + targetDevice); //$NON-NLS-1$
      }
   }
